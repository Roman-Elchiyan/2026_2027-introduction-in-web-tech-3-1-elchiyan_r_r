...
Развернуть в Docker систему мониторинга на базе Prometheus, Node Exporter и Grafana, настроить сбор и визуализацию системных метрик, а затем выполнить базовый анализ безопасности учебного веб-приложения OWASP Juice Shop в локальной среде.

Все проверки безопасности выполнялись только на локально запущенном OWASP Juice Shop — приложении, специально предназначенном для обучения и тестирования уязвимостей.
...
### 2.3. Анализ `robots.txt` и каталога `/ftp`

Запрос к `robots.txt` показал следующую запись:
...
`robots.txt` не ограничивает доступ к ресурсу, а только сообщает поисковым роботам, какие пути не следует индексировать. Поэтому указанный в нём путь можно открыть напрямую.

В каталоге `/ftp` отображался список файлов, включая служебные документы и резервные копии.

![Каталог ftp](images/07_ftp_listing.png)

*Рисунок 7 — Список файлов, доступных в каталоге `/ftp`.*

При открытии одного из текстовых документов сервер вернул `HTTP 200 OK` и содержимое файла. Это показывает риск размещения служебных файлов в каталоге с публичным доступом.

![Получение документа из ftp](images/08_exposed_file_response.png)

*Рисунок 8 — Получение документа из каталога `/ftp`.*

### 2.4. Проверка обработки некорректного пути

Дополнительно была проверена обработка некорректного пути. Сервер вернул `HTTP 200 OK`, но в теле ответа находилась обычная HTML-страница OWASP Juice Shop. Содержимое системного файла получено не было.

Следовательно, код `200 OK` сам по себе не подтверждает успешное чтение файла: необходимо анализировать содержимое ответа.

![Ответ сервера при проверке некорректного пути](images/09_path_traversal_200_ok.png)

*Рисунок 9 — Ответ сервера `HTTP 200 OK` при проверке обработки некорректного пути.*

> **Важно:** перед загрузкой отчёта на GitHub заменить файл `images/09_path_traversal_200_ok.png` своим скриншотом. Имя файла оставьте без изменений.
